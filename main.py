...
def initialize_draw_surface(w, h):
    global draw_surface
    draw_surface = pygame.Surface((w, h))
    draw_surface.fill((255, 255, 255))

def draw_brush(p0, p1):
    # Draw a line with round caps between two points on draw_surface
    if draw_surface is None:
        return
    x0, y0 = p0
    x1, y1 = p1
    pygame.draw.line(draw_surface, (0, 0, 0), (x0, y0), (x1, y1), brush_size)
    # Round caps by drawing circles at ends
    pygame.draw.circle(draw_surface, (0, 0, 0), (int(x0), int(y0)), max(1, brush_size // 2))
    pygame.draw.circle(draw_surface, (0, 0, 0), (int(x1), int(y1)), max(1, brush_size // 2))
...
# Source mode: image import or drawing canvas
source_mode = 'image'  # 'image' or 'draw'
draw_surface = None  # initialized lazily to fit the ASCII panel view
is_drawing = False
brush_size = 8
_prev_draw_pos = None
...
# Source toggles (Image / Draw) placed below Save/Copy/Clear row
# Initial placement; will be updated dynamically each frame
_source_y = clear_button_rect.bottom + 10
source_image_rect = pygame.Rect(CONTROL_X, _source_y, 100, 32)
source_image_text = font.render('Image', True, COLOR_TEXT)
source_draw_rect = pygame.Rect(CONTROL_X + 104, _source_y, 100, 32)
source_draw_text = font.render('Draw', True, COLOR_TEXT)
brush_label_text = font.render('Brush', True, COLOR_TEXT_MUTED)
# Brush controls on a separate row to avoid overlap
_brush_y = _source_y + 44
# Layout: [Brush label] [ - ] [ value ] [ + ] [ preview]
brush_minus_rect = pygame.Rect(CONTROL_X + 100, _brush_y, 32, 32)
brush_plus_rect = pygame.Rect(CONTROL_X + 180, _brush_y, 32, 32)
_brush_value_x = CONTROL_X + 140
# preview center x coordinate
_brush_preview_x = CONTROL_X + 235
def _render_brush_value():
    return font.render(str(brush_size), True, COLOR_TEXT)

def update_layout_positions():
    """Reflow sidebar with order: source -> Import -> Generate -> mode -> brush -> sliders -> aspect -> save/copy/clear."""
    global _source_y, _brush_y
    y = input_box.bottom + 8
    # Source toggles (Image/Draw)
    _source_y = y
    source_image_rect.y = _source_y
    source_draw_rect.y = _source_y
    y = _source_y + 40
    # Import Image button
    button_rect.y = y
    y = button_rect.y + 56
    # Generate ASCII Art button
    generate_button_rect.y = y
    y = generate_button_rect.y + 56
    # Mode toggles (Word/Dense) BELOW Generate per request
    word_mode_rect.y = y
    density_mode_rect.y = y
    # Align x with Image/Draw buttons
    word_mode_rect.x = source_image_rect.x
    density_mode_rect.x = source_draw_rect.x
    y = y + 44
    # Brush row
    _brush_y = y
    brush_minus_rect.y = _brush_y
    brush_plus_rect.y = _brush_y
    # Extra spacing after brush row to separate from sliders
    y = _brush_y + 64
    # Sliders
    width_slider_rect.y = y
    y += 56
    height_slider_rect.y = y
    y += 56
    density_slider_rect.y = y
    y += 56
    zoom_slider_rect.y = y
    y += 56
    panx_slider_rect.y = y
    y += 56
    pany_slider_rect.y = y
    y += 56
    # Aspect lock checkbox and label
    aspect_checkbox_rect.y = y
    y += 44
    # Save / Copy / Clear row under aspect lock
    save_button_rect.y = y
    copy_button_rect.y = y
    clear_button_rect.y = y
...
# Generate ASCII art (accept path, PIL.Image, or pygame.Surface)
def generate_ascii_art(image_source, word_text, art_width, art_height):
    # Resolve image_source into a grayscale PIL image
    if isinstance(image_source, Image.Image):
        img = image_source.convert('L')
    elif isinstance(image_source, pygame.Surface):
        w, h = image_source.get_size()
        raw = pygame.image.tostring(image_source, 'RGB')
        img = Image.frombytes('RGB', (w, h), raw).convert('L')
    else:
...
def update_ascii_art():
    ...
    # Determine if we have a valid source based on mode
    has_source = (source_mode == 'image' and image_path) or (source_mode == 'draw' and draw_surface is not None)
    if has_source and text and art_width >= 10 and art_height >= 5:
        ascii_font = get_ascii_font(art_width)
        src = draw_surface if source_mode == 'draw' else image_path
        ascii_art = generate_ascii_art(src, text, art_width, art_height)
...
def redraw_screen():
    ...
    # Update sidebar layout to requested order
    ...
    # Source toggles
    draw_button(screen, source_image_rect, source_image_text, hovered=source_image_rect.collidepoint(mouse_pos), active=(source_mode == 'image'))
    draw_button(screen, source_draw_rect, source_draw_text, hovered=source_draw_rect.collidepoint(mouse_pos), active=(source_mode == 'draw'))

    # Brush controls row (label, -, value, +, preview)
    screen.blit(brush_label_text, (CONTROL_X, _brush_y + (32 - brush_label_text.get_height()) // 2))
    draw_button(screen, brush_minus_rect, font.render('-', True, COLOR_TEXT), hovered=brush_minus_rect.collidepoint(mouse_pos))
    val_surface = _render_brush_value()
    screen.blit(val_surface, (_brush_value_x, _brush_y + (32 - val_surface.get_height()) // 2))
    draw_button(screen, brush_plus_rect, font.render('+', True, COLOR_TEXT), hovered=brush_plus_rect.collidepoint(mouse_pos))
    # Brush preview circle (approx radius)
    preview_center = (_brush_preview_x, _brush_y + 16)
    pygame.draw.circle(screen, COLOR_TEXT_MUTED, preview_center, max(2, brush_size // 2), width=1)

    # (Draw mode instructions removed by request)
    ...
    if ascii_canvas:
        ...
        # If no ASCII yet and in draw mode, show the drawing surface as canvas
        if source_mode == 'draw':
            view = pygame.Rect(ascii_panel_rect.x + padding, ascii_panel_rect.y + padding,
                               ascii_panel_rect.width - 2 * padding, ascii_panel_rect.height - 2 * padding)
            if draw_surface is None or draw_surface.get_size() != (view.width, view.height):
                # Initialize or resize draw surface (white background)
                initialize_draw_surface(view.width, view.height)
            screen.blit(draw_surface, (view.x, view.y))
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            # Begin drawing if in draw mode and clicking inside the view
            if source_mode == 'draw':
                padding = 16
                view = pygame.Rect(ascii_panel_rect.x + padding, ascii_panel_rect.y + padding,
                                   ascii_panel_rect.width - 2 * padding, ascii_panel_rect.height - 2 * padding)
                if draw_surface is None or draw_surface.get_size() != (view.width, view.height):
                    initialize_draw_surface(view.width, view.height)
                if view.collidepoint(event.pos) and event.button == 1:
                    is_drawing = True
                    _prev_draw_pos = (event.pos[0] - view.x, event.pos[1] - view.y)
                    redraw_needed = True
            ...
            if button_rect.collidepoint(event.pos):
                if source_mode == 'image':
                    ...
                    # In draw mode, use button as Clear Drawing for convenience
                    if draw_surface is not None:
                        draw_surface.fill((255, 255, 255))
                        ascii_art = []
                        ascii_art_surfaces = []
                        ascii_canvas = None
                        status_message = "Drawing cleared."
                redraw_needed = True
            ...
            if generate_button_rect.collidepoint(event.pos):
                if source_mode == 'image' and not image_path:
                    ...
                    if art_width < 10 or art_height < 5:
                        ...
                        update_ascii_art()
                        status_message = "ASCII art generated from {}!".format('drawing' if source_mode == 'draw' else 'image')
                redraw_needed = True
            # Save button
            ...
            if clear_button_rect.collidepoint(event.pos):
                ...
                if source_mode == 'draw' and draw_surface is not None:
                    draw_surface.fill((255, 255, 255))
                redraw_needed = True
            # Mode toggles
            ...
            if density_mode_rect.collidepoint(event.pos):
                ...
                update_ascii_art(); redraw_needed = True
            # Source toggles
            if source_image_rect.collidepoint(event.pos):
                source_mode = 'image'
                redraw_needed = True
            if source_draw_rect.collidepoint(event.pos):
                source_mode = 'draw'
                # ensure draw surface exists
                padding = 16
                view = pygame.Rect(ascii_panel_rect.x + padding, ascii_panel_rect.y + padding,
                                   ascii_panel_rect.width - 2 * padding, ascii_panel_rect.height - 2 * padding)
                if draw_surface is None or draw_surface.get_size() != (view.width, view.height):
                ...
                redraw_needed = True
            # Brush size buttons
            if brush_minus_rect.collidepoint(event.pos):
                brush_size = max(1, brush_size - 1)
                status_message = f"Brush: {brush_size}"
                redraw_needed = True
            if brush_plus_rect.collidepoint(event.pos):
                brush_size = min(64, brush_size + 1)
                status_message = f"Brush: {brush_size}"
                redraw_needed = True
        if event.type == pygame.MOUSEBUTTONUP:
            if source_mode == 'draw' and event.button == 1:
                is_drawing = False
                _prev_draw_pos = None
            width_dragging = False
            height_dragging = False
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            if pany_dragging:
                ...
                redraw_needed = True
            # Drawing with mouse while in Draw mode (only when no ASCII preview)
            if source_mode == 'draw' and ascii_canvas is None:
                padding = 16
                view = pygame.Rect(ascii_panel_rect.x + padding, ascii_panel_rect.y + padding,
                                   ascii_panel_rect.width - 2 * padding, ascii_panel_rect.height - 2 * padding)
                if is_drawing and draw_surface is not None and view.collidepoint(event.pos):
                    x, y = event.pos[0] - view.x, event.pos[1] - view.y
                    draw_brush((_prev_draw_pos or (x, y)), (x, y))
                    _prev_draw_pos = (x, y)
                    redraw_needed = True
